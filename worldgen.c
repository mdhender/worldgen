...
void main(int argc, char **argv)
{
	...
	for (i = 0; i < XRange; i++) {
		SinIterPhi[i] = SinIterPhi[i + XRange] =
		    (float) sin(i * 2 * PI / XRange);
	}
	...
	for (j = 0, row = 0; j < XRange / 2; j++) {
		for (i = 1; i < YRange; i++) {	/* fix */
			WorldMapArray[row + index2 + YRange - i] =
			    WorldMapArray[row + i];
		}
		row += YRange;
	}
	...
	for (j = 0, row = 0; j < XRange; j++) {
		for (i = 0; i < YRange; i++) {
			...
			Color =
			    (int) (((float) (Color - MinZ + 1) /
				    (float) (MaxZ - MinZ + 1)) * 30) + 1;
			Histogram[Color]++;
		}
		row += YRange;
	}
...
void GenerateWorldMap()
{
	...
	for (Phi = 0; Phi < XRange / 2; Phi++) {
		Theta =
		    (int) (YRangeDivPI *
			   atan(*(SinIterPhi + Xsi - Phi + XRange) *
				TanB)) + YRangeDiv2;
...
static void compress(init_bits, outfile)
int init_bits;
FILE *outfile;
{
	register long fcode;
